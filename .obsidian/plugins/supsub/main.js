...
var supSubDecorationPlugin = import_view.ViewPlugin.define(
	(view) => {
		return {
...
var SupSubSettingTab = class extends import_obsidian.PluginSettingTab {
	...
	display() {
		...
		new import_obsidian.Setting(containerEl)
			...
			.setDesc(
				"Toggle the visibility of the SupSub popup buttons when text is selected.",
			)
			.addToggle((toggle) =>
				toggle
					...
					.onChange(async (value) => {
						...
						new import_obsidian.Notice(
							`Popup Buttons ${value ? "Enabled" : "Disabled"}`,
						);
						if (!value) {
		...
		new import_obsidian.Setting(containerEl)
			...
			.setDesc(
				"Instantly hide the <sup> and <sub> tags in Editor Mode after wrapping.",
			)
			.addToggle((toggle) =>
				toggle
					.setValue(this.plugin.settings.hideTags)
					.onChange(async (value) => {
						this.plugin.settings.hideTags = value;
						await this.plugin.saveSettings();
						new import_obsidian.Notice(
							`Hide Tags ${value ? "Enabled" : "Disabled"}`,
						);
...
var SupSubPlugin = class extends import_obsidian.Plugin {
	...
	async onload() {
		...
		if (this.settings.hideTags) {
			this.supSubDecorations = this.registerEditorExtension(
				supSubDecorationPlugin,
			);
		}
	}
	onunload() {
	...
	showSupSubButtons(editor) {
		if (!this.settings.enablePopup) return;
		this.hideSupSubButtons();
		const selection = editor.getSelection();
		if (!selection) return;
		...
		if (currentTag === "sup" || currentTag === "sub") {
			...
			supButton.setAttribute(
				"aria-label",
				"Wrap selected text with superscript",
			);
	...
	positionPopup(popup, editor) {
		...
		if (selection && selection.rangeCount > 0) {
			...
			console.log(
				`Popup positioned at top: ${top}px, left: ${calculatedLeft}px`,
			);
		}
	}
		...
		try {
			if (this.selectionStart && this.selectionEnd) {
				...
				const isValidStart =
					...
					this.selectionStart.line < docLines &&
					this.selectionStart.ch >= 0 &&
					this.selectionStart.ch <=
						editor.getLine(this.selectionStart.line).length;
				const isValidEnd =
					this.selectionEnd.line >= 0 &&
					this.selectionEnd.line < docLines &&
					this.selectionEnd.ch >= 0 &&
					this.selectionEnd.ch <= editor.getLine(this.selectionEnd.line).length;
				if (isValidStart && isValidEnd) {
					editor.setSelection(this.selectionStart, this.selectionEnd);
				} else {
					console.warn(
						"Invalid selection points. Clearing selectionStart and selectionEnd.",
					);
					this.selectionStart = null;
					this.selectionEnd = null;
				}
	...
	optimizeTags(line, tag) {
		const openTag = `<${tag}>`;
		const closeTag = `</${tag}>`;
		let optimizedLine = line.replace(
			new RegExp(`(${closeTag})(${openTag})`, "g"),
			"",
		);
		optimizedLine = optimizedLine.replace(
			new RegExp(`\\${openTag}\\s*\\${closeTag}`, "g"),
			"",
		);
